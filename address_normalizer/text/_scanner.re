...
possible_word_char = {letter}|{mark_spacing_combining_chars}|{mark_enclosing_chars}|{mark_nonspacing_chars}|{punct_connector_chars}|{non_breaking_dash}|{currency_symbol_chars}|{symbol_modifier_chars}|{symbol_math_chars}|{symbol_other_chars}|{digit};
...
PyObject *py_scan(PyObject *self, PyObject *args) 
{
    ...
    PyObject *str = PyUnicode_AsEncodedString((PyObject *)unistr, "utf-8", "strict");
    if (str == NULL) {
        PyErr_SetString(PyExc_TypeError,
                        "Parameter could not be utf-8 encoded");
    }

    char *input = PyString_AsString(str);
    ...
    unsigned char *end = start + strlen(input) + 1;

    Scanner scanner;

    scanner.src = (unsigned char *)input;
    ...
    while ( (token_type = scan(&scanner)) != END ) {
        ...
        if (token_type != WHITESPACE) { 
            token = PyUnicode_FromStringAndSize(input + token_start, (Py_ssize_t) token_length);
            item = Py_BuildValue("iO", token_type, token);
