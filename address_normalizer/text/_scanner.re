...
#define NUMERIC 51
#define ORDINAL 52
#define NUMERIC_RANGE 53
#define ORDINAL_RANGE 54
#define ROMAN_NUMERAL 55
...
#define LBSQUARE 115
#define RBSQUARE 116
#define DOUBLE_QUOTE 117
#define SINGLE_QUOTE 118
#define LEFT_DOUBLE_QUOTE 119
#define RIGHT_DOUBLE_QUOTE 120
#define LEFT_SINGLE_QUOTE 121
#define RIGHT_SINGLE_QUOTE 122
#define SLASH 123
#define BACKSLASH 124
#define GREATER_THAN 125
#define LESS_THAN 126
...
typedef struct Scanner {
    Py_UNICODE *src, *cursor, *start, *end;
} Scanner;
...
non_space = [^ \u00a0\u1680\u180e\u2000-\u200a\u202f\u3000\u0000-\u001f\u007f-\u009f];
letter = {lowercase_letter_chars}|{uppercase_letter_chars};
...
any_word = ({letter}|{possible_word_char})*({letter}{possible_word_char})|({possible_word_char}{letter})({letter}|{possible_word_char})*;
basic_word = ({digit}|{letter})*{letter}+({digit}|{letter})*;
hyphen_word = {basic_word}{hyphen}{basic_word};

apos_word = ("'"?({letter}+"'")+{letter}+"'"?);
...
numeric = ({digit}|{possible_word_char})*{digit}+({digit}|{possible_word_char})*;

number = ("\-"?{digit}+",")*{digit}+;

ordinal = {digit}+("st"|"nd"|"rd"|"th");

numeric_range = ({digit}+"\-"{digit}+);
ordinal_range = ({ordinal}|{digit}+)"\-"{ordinal};
...
{ordinal_range}					{ return ORDINAL_RANGE; }
{numeric_range}					{ return NUMERIC_RANGE; }

{multi_punct_abbreviation}      { return ABBREVIATION; }
{hyphen_plus_abbreviation}      { return ABBREVIATION; }
{letter}+"\."					{ return ABBREVIATION; }
...
PyObject *py_scan(PyObject *self, PyObject *args) 
{
    ...
    PyObject *arg1;
    if (!PyArg_ParseTuple(args, "O:scanner", &arg1)) {
        return 0;
    }
    ...
    int token_type;
    while ( (token_type = scan(&scanner)) != END ) {
        token_start = scanner.start - scanner.src;
        token_length = scanner.cursor-scanner.start;
        if (token_type != WHITESPACE) { 
            PyObject *item = Py_BuildValue("iii", token_type, token_start, token_length);
...
PyMODINIT_FUNC init_scanner(void) {
    (void) Py_InitModule("_scanner", module_functions);
}
